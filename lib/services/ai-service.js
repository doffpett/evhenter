 ...
 * Handles AI interactions for URL parsing and image generation
 * Uses Vercel AI Gateway for unified access to multiple providers
 */

import { generateObject, generateText } from 'ai';
import { z } from 'zod';

// Vercel AI Gateway automatically uses AI_GATEWAY_API_KEY env var
// No need to import specific providers - just use model strings like 'openai/gpt-4o-mini'
...
export async function parseEventFromUrl(url, htmlContent = null) {
  try {
    ...
    // Use Vercel AI Gateway with model string format
    const result = await generateObject({
      model: 'openai/gpt-4o-mini', // Uses AI Gateway
      schema: eventSchema,
      prompt: `
...
No text, no people, just abstract artistic representation.
    ...
    // Use OpenAI client through AI Gateway
